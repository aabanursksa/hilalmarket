  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-xl border border-border bg-white transition hover:shadow-lg">
      ...
      <div className="absolute right-3 top-3 z-10 flex flex-col gap-1.5">
        <button onClick={() => toggleItem(p)} className={`flex h-8 w-8 items-center justify-center rounded-full shadow transition ${fav ? "bg-brand text-white" : "bg-white hover:bg-brand hover:text-white"}`}>
      ...
      <div className="flex flex-1 flex-col gap-1 p-3">
        ...
        <h3 className="line-clamp-2 text-sm font-semibold text-foreground min-h-[2.5rem]">{p.name}</h3>
        <div className="mt-2 flex items-center gap-2">
          <button onClick={() => addItem(p)} className="flex h-9 flex-1 items-center justify-center gap-1.5 rounded-full bg-brand text-xs font-bold text-white transition hover:bg-brand/90">
            <ShoppingCart className="h-3.5 w-3.5" /> Add to cart
          </button>
          <div className="flex items-center rounded-full border border-border">
            <button onClick={() => setQty(Math.max(1, qty - 1))} className="px-2 py-1.5 text-foreground hover:text-brand"><Minus className="h-3 w-3" /></button>
            <span className="px-1 text-xs font-bold text-foreground whitespace-nowrap">{qty}{p.unit.replace(/\d+/, "")}</span>
            <button onClick={() => setQty(qty + 1)} className="px-2 py-1.5 text-foreground hover:text-brand"><Plus className="h-3 w-3" /></button>
          </div>
        </div>
      </div>
    </div>
